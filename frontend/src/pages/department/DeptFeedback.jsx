...
export default function DeptFeedback() {
  const [feedbacks, setFeedbacks] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    api.get('/feedback/department').then(res => setFeedbacks(res.data)).catch(() => {}).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <div>
      <div className="page-header">
        <h1 className="page-title">Department Feedback</h1>
        <p className="page-subtitle">{feedbacks.length} feedback(s) • Average: ⭐ {avg}</p>
      </div>
      {feedbacks.length > 0 ? (
          ...
          {feedbacks.map(f => (
            <div key={f.id} className="glass-card">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 'var(--space-sm)' }}>
                <div>
                  <span style={{ fontWeight: 600 }}>{f.citizenName}</span>
